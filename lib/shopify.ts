...
export const searchProducts = async (query: string): Promise<Product[]> => {
  try {
    ...
    // Construct query: title:*term1* OR title:*term2* ...
    ...
    const queryString = `(${titleQueries}) OR (${tagQueries}) OR (${typeQueries})`;
    
    const products = await client.product.fetchQuery({ query: queryString, first: 50 });
    return products.map(normalizeProduct);
  } catch(e) {
