...
const extractWarranty = (tags: string[], description: string): string => {
  // 1. Check tags for warranty:X
  const warrantyTag = tags.find(tag => tag.toLowerCase().startsWith('warranty:'));
  if (warrantyTag) {
    return warrantyTag.split(':')[1].trim();
  }
  ...
  return "1 Year"; // Default if not found
};
...
export const searchProducts = async (query: string): Promise<Product[]> => {
  ...
  try {
    ...
    return products.sort((a, b) => {
      ...
      const getScore = (title: string, category: string, tags: string[]) => {
        let score = 0;
        
        // 1. Exact or near-exact title match
        if (title === lowerQuery) score += 2000;
        else if (title.includes(lowerQuery)) score += 1000;
        
        // 2. Word-based matching (crucial for long names)
        const queryWords = lowerQuery.split(/\s+/).filter(w => w.length > 2);
        const titleWords = title.split(/\s+/);
        
        let matchingWordCount = 0;
        queryWords.forEach(qw => {
          if (title.includes(qw)) matchingWordCount++;
        });
        ...
        if (queryWords.length > 0) {
          ...
          if (matchingWordCount === queryWords.length) score += 300;
        }

        // 3. Category match
        if (category === lowerQuery || category.includes(lowerQuery)) score += 100;
        
        // 4. Tag match
        if (tags.some(t => t.toLowerCase().includes(lowerQuery))) score += 50;

        return score;
      };

      const scoreA = getScore(aTitle, aCat, a.tags);
      const scoreB = getScore(bTitle, bCat, b.tags);
